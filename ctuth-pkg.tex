...
\cs_new:Nx \ctuthesis_geometry_hmarginratio: {5\string :4} % hanetar 5:4 (default 4:6)

% Target textwidth is 33*basesize (10pt to 12pt)
\dim_const:Nn \g_ctuthesis_geometry_textwidth_dim { \tl_use:N \g_ctuthesis_fontsize_tl * 44 } % hanetar 44 (default 33)
...
\ctuifswitch { layout-short } {
	...
	\dim_const:Nn \g_ctuthesis_geometry_textheight_dim { 0.8\paperheight } % hanetar 0.8 (default 0.75)
	\cs_new:Nx \ctuthesis_geometry_vmarginratio: {1\string :1} % hanetar 1:1 (default 5:6)
}
...
\cs_new_protected:Nn \ctuthesis_geometry_title: {
	\newgeometry {
		...
		textwidth = \g_ctuthesis_geometry_textwidth_dim, % hanetar, (default \dim_min:nn { 420pt } { 1.2 \g_ctuthesis_geometry_textwidth_dim } ),
	 	textheight = \g_ctuthesis_geometry_textheight_dim ,
...
\def\ttlh@ctuch#1#2#3#4#5#6#7#8{
	...
	% \ifttl@label\else\addcontentsline{toc}{chapter}{#8}\fi % hanetar commented out to hide abstract, thanks, etc from ToC
	\ctu@title@boxify{#5}{#3}{
		{#1\strut\ifttl@label#2\fi\par}
		\medskip
		{\color{ctubluetext}#4\strut#8\strut\par}
...
\setcounter{tocdepth}{1} % hanetar 1 (default 2)
